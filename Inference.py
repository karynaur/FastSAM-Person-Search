...
def parse_args():
    ...
    parser.add_argument(
        "--query", type=str, default=None, help='query image path, e.g. "dogs.jpg"'
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.box_prompt[2] != 0 and args.box_prompt[3] != 0:
        ...
        annotations = prompt(results, args, query=True)
        annotations = np.array([annotations])
        fast_process(
...
def prompt(results, args, box=None, point=None, query=None):
    ...
    if box:
        ...
        mask, _ = query_prompt(results, args.query, args.img_path,args.device)
    else:
        return None
    return mask
