...
@torch.no_grad()
def retriev(
    model, preprocess, elements, query, device
) -> int:
    preprocessed_images = [preprocess(image).to(device) for image in elements]
    preprocessed_query = preprocess(Image.open(query)).to(device)
    stacked_images = torch.stack(preprocessed_images)
    image_features = model.encode_image(stacked_images)
    query_features = model.encode_image(preprocessed_query)
    image_features /= image_features.norm(dim=-1, keepdim=True)
    query_features /= query_features.norm(dim=-1, keepdim=True)
    probs = 100.0 * image_features @ query_features.T
...
def query_prompt(annotations, query, img_path,device):
    cropped_boxes, cropped_images, not_crop, filter_id, annotaions = crop_image(
        annotations, img_path
    )
    clip_model, preprocess = clip.load("ViT-B/32", device=device)
    scores = retriev(
        clip_model, preprocess, cropped_boxes, query, device=device
    )
